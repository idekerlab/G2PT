...
from src.utils.data.dataset import SNP2PCollator, PLINKDataset, BlockQueryDataset, BlockDataset
...
def main():
    ...
    tree_parser = SNPTreeParser(args.onto, args.snp2gene, by_chr=False, sys_annot_file=args.system_annot, multiple_phenotypes=True)
    ...
    dataset = PLINKDataset(tree_parser, args.bfile,
                                 args.cov,
                                 args.pheno, flip=g2p_model_dict['arguments'].flip,
                                 input_format=g2p_model_dict['arguments'].input_format,
                                 cov_ids=g2p_model_dict['arguments'].cov_ids, pheno_ids=g2p_model_dict['arguments'].pheno_ids,
                                 bt=g2p_model_dict['arguments'].bt, qt=g2p_model_dict['arguments'].qt)
    collator = SNP2PCollator(tree_parser, input_format=g2p_model_dict['arguments'].input_format)
    ...
    snp2p_dataset = PhenotypeSelectionDataset(tree_parser, dataset, collator, args.batch_size, shuffle=False)
    print(args.phenotypes)
    if type(args.phenotypes) != list:
        args.phenotypes = [args.phenotypes]
    snp2p_dataset.select_phenotypes(args.phenotypes)
    #dataset = SNP2PDataset(whole_df, genotypes, tree_parser, n_cov=args.n_cov, age_mean=age_mean, age_std=age_std)
    device = torch.device("cuda:%d"%args.cuda)
    whole_dataloader = DataLoader(snp2p_dataset, batch_size=None, num_workers=0)
    ...
    for k, v in g2p_model_dict['state_dict'].items():
    ...
    g2p_model = SNP2PhenotypeModel(tree_parser, hidden_dims=g2p_model_dict['arguments'].hidden_dims,
                                   ...
                                   input_format=g2p_model_dict['arguments'].input_format,
                                   cov_effect=g2p_model_dict['arguments'].cov_effect)

    g2p_model.load_state_dict(new_state_dict)
    ...
    for i, batch in enumerate(tqdm(whole_dataloader)):
        ...
        with torch.no_grad():
            hierarchical_mask_forward = batch['genotype']['hierarchical_mask_forward']
            hierarchical_mask_backward = batch['genotype']['hierarchical_mask_backward']
    ...
    for ind, pheno in enumerate(args.phenotypes):
