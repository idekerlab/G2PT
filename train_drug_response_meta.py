...
from src.utils.data.dataset import DrugResponseDataset, DrugResponseCollator, DrugDataset, CellLineBatchSampler, MetaDrugResponseDataset, DistributedMetaTaskSampler
from src.utils.trainer import DrugResponseTrainer, DrugTrainer, MetaDrugResponseTrainer
import numpy as np
...
def main():
    ...
    parser.add_argument('--inner_lr', help='Learning rate', type=float, default=0.001)
    parser.add_argument('--outer_lr', help='Learning rate', type=float, default=0.1)
    parser.add_argument('--inner_loop_steps', help='Learning rate', type=int, default=10)

    parser.add_argument('--n_support', help='Learning rate', type=int, default=10)
    parser.add_argument('--n_query', help='Learning rate', type=int, default=5)
    parser.add_argument('--n_class', help='Learning rate', type=int, default=5)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    drug_response_dataset = MetaDrugResponseDataset(train_dataset, args.cell2id, args.genotypes, compound_encoder,
                                                tree_parser, mut2gene=args.mut2gene, with_indices=args.with_indices, class_ind=4)


    drug_response_collator = DrugResponseCollator(tree_parser, list(args.genotypes.keys()), compound_encoder,
                                                  mut2gene=args.mut2gene, with_indices=args.with_indices)

    task_sampler = DistributedMetaTaskSampler(drug_response_dataset, drug_response_collator, args.n_class, args.n_support, args.n_query, args.rank, args.world_size)
    if args.distributed:
    ...
    drug_response_trainer = MetaDrugResponseTrainer(drug_response_model, tree_parser, task_sampler,
                                                    device, args, validation_dataloader=val_drug_response_dataloader, fix_embedding=fix_embedding)
