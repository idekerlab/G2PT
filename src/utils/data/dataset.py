...
class G2PDataset(Dataset):
    ...
    def __init__(self, genotype_phenotype, cell2genotypes, tree_parser:TreeParser):
        self.g2p_df = genotype_phenotype
        self.tree_parser = TreeParser
        self.cell2genotype_dict = {genotype: pd.read_csv(mut_data, header=None) for genotype, mut_data in
    ...
    def __getitem__(self, index):
        ...
        cell_mut_dict = {mut_type:self.tree_parser.get_system2genotype_mask(torch.tensor(mut_value.iloc[int(cell_ind)].values, dtype=torch.float32))
...
class G2PCollator(object):
    ...
    def __call__(self, data):
        ...
        result_dict['phenotype'] = torch.tensor([dr['phenotype'] for dr in data], dtype=torch.float32)
        return result_dict

class DrugResponseDataset(Dataset):

    def __init__(self, drug_response, cell2ind, cell2genotypes, compound_encoder, tree_parser:TreeParser):
        ...
        self.cell2genotype_dict = {genotype: pd.read_csv(mut_data, header=None) for genotype, mut_data in cell2genotypes.items()}
    ...
    def __getitem__(self, index):
        ...
        cell_mut_dict = {mut_type:self.tree_parser.get_system2genotype_mask(torch.tensor(mut_value.iloc[cell_ind].values, dtype=torch.float32))
                         for mut_type, mut_value in self.cell2genotype_dict.items()}
        #cell_tree_mask = self.cell_tree_mask_dict[cell]

        result_dict = dict()
        result_dict['genotype'] = cell_mut_dict
        result_dict['drug'] = self.compound_encoder.encode(drug)
...
class DrugResponseCollator(object):
    ...
    def __init__(self, genotypes, compound_encoder):
        self.genotypes = genotypes
        self.compound_encoder = compound_encoder
...
'''
    def __iter__(self):
        count = 0
        shuffle(self.celllines)
        while count < self.n_celllines:
            celline_df = self.cellline_grouped.get_group(self.celllines[count])
            index = np.random.random_integers(0, celline_df.shape[0] - 1, size=self.batch_size)
            yield celline_df.iloc[index].index.tolist()
            count += 1
