...
class GreedyMultiplePhenotypeTrainer(SNP2PTrainer):
    ...
    def greedy_phenotype_selection(self) -> Tuple[float, nn.Module, List[str]]:
        ...
        for i in range(50):
            self.iter_minibatches(model2train, snp2p_dataloader, optimizer, epoch=i, name=f"Warmup with target phenotypes, {','.join(phenotype_set) }")
        ...
        #initial_performance, initial_model = self.train_with_phenotypes(self.snp2p_model, phenotype_set, epoch=0)
        prev_performance = initial_performance
        train_dataset = self.snp2p_dataloader.dataset  # .select_phenotypes(phenotypes)
        train_dataset.select_phenotypes(phenotype_set)
        #snp2p_dataloader = DataLoader(train_dataset, batch_size=None, num_workers=self.args.jobs, prefetch_factor=2)
        while True:
            candidate_performance, candidate_model = self.train_with_phenotypes(model2train, phenotype_set, balanced=False)
            improvement = candidate_performance - prev_performance
            if improvement < self.improvement_threshold:
                self.logger.info(
                    f"Improvement {improvement:.4f} below threshold {self.improvement_threshold} for initial model. Stopping.")
                break

        initial_performance = candidate_performance
        initial_model = candidate_model
        self.save_model(initial_model, phenotype_set)
        # Track history
            ...
            if phenotype not in phenotype_set:
                ...
                # Train with new phenotype set
                    temporal_performance, temporal_model = self.train_with_phenotypes(
                        temporal_model, temporal_phenotype_set, epoch=i, balanced=balanced)
    ...
    '''
    def train_with_phenotypes(self, model, phenotypes, epoch=0, balanced=False):
        train_dataset = self.snp2p_dataloader.dataset#.select_phenotypes(phenotypes)
        if balanced:
        ...
        optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model2train.parameters()), lr=self.args.lr/10,
                                     weight_decay=self.args.wd)
        self.iter_minibatches(model2train, snp2p_dataloader, optimizer, epoch=epoch, name=f"Training with phenotypes, {','.join(phenotypes) }")
        performance = self.evaluate(model2train, val_snp2p_dataloader, 0, phenotypes=phenotypes, name=f"Validation with phenotypes, {','.join(phenotypes) }")
    ...
    def save_model(self, model, phenotype_set):
        if self.args.out:
            output_path = self.args.out + ".".join(phenotype_set) + ".pt"
