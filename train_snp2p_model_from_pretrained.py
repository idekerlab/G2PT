...
from src.utils.data.dataset import SNP2PCollator, BlockDataset, BlockQueryDataset
...
def main():
    ...
    parser.add_argument('--freeze-pretrained', help='Training covariates dataset', action='store_true', default=False)
    parser.add_argument('--val-bfile', help='Training genotype dataset', type=str, default=None)
...
def main_worker(args):
    ...
    for chromosome, block in blocks:
        block_bfile = BlockDataset(bfile=f'/cellar/users/i5lee/G2PT_T2D/genotype_data/LD_blocks_HapMap/split_block_chr{chromosome}_block{block}')
    ...
    snp2p_dataset = BlockQueryDataset(tree_parser, args.train_bfile, block_bfile_dict, args.train_cov, args.train_pheno,
    ...
    if args.val_cov is not None:
        val_snp2p_dataset = BlockQueryDataset(tree_parser, args.val_bfile, block_bfile_dict, args.val_cov, args.val_pheno, cov_mean_dict=args.cov_mean_dict,
